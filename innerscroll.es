...
function init (evt) {
	...
	viewboxW = Number(main_viewbox[2]);
	viewboxH = Number(main_viewbox[3]);
	viewbox_ratio = viewboxW/viewboxH;
	loop = null;
	v_disp = 0;
	ln_scrl = 14;
	pg_scrl = svgdoc.getElementById("background").getAttributeNS(null,"height") - 20;
	innerwinH = parseFloat(svgdoc.getElementById("container").getAttributeNS(null,"height"));
	innerwinY = parseFloat(svgdoc.getElementById("container").getAttributeNS(null,"y"));
	var scrollbarH = parseFloat(svgdoc.getElementById("scrollbar").getAttributeNS(null,"height"));
	markY = parseFloat(svgdoc.getElementById("mark").getAttributeNS(null,"y"));
	newMarkY = markY;
	win_bar_ratio = innerwinH / scrollbarH;
	markH = scrollbarH / win_bar_ratio - 20 / win_bar_ratio;
	adjusted = innerwinH / (scrollbarH - markH);
	svgdoc.getElementById("mark").setAttributeNS(null,"height",markH);
	maxY = markY + scrollbarH - markH;
	firstiteration = true;
	reset();
}
